...
我们知道在平直的 Euclid 空间中, 两点间的距离可以用勾股定理表出:
...
虽然勾股定理在不同空间和坐标系下都成立, 但它们的形式似乎不太统一（如 $\eqref{eq1}$、$\eqref{eq2}$ 和 $\eqref{eq3}$）。但它们每项都是平方项。这促使我们想到线性代数中所学过的二次型。

于是我们尝试将上述三个勾股定理写成二次型矩阵的形式:
...
于是我们就把矩阵 $g_{ab}$ 定义为**度规（metric）**。那么上面三个例子以及其它空间 / 坐标中的度规就一目了然啦 :P。
