...
雖然勾股定理在不同空間和座標系下都成立，但它們的形式似乎不太統一（如 $\eqref{eq1}$、$\eqref{eq2}$ 和 $\eqref{eq3}$）。然而它們每項都是平方項。這促使我們想到線性代數中所學過的二次型。

於是我們嘗試將上述三個勾股定理寫成二次型矩陣的形式：
...
於是我們就把矩陣 $g_{ab}$ 定義為**度規（metric）**。那麼上面三個例子以及其它空間 / 坐標中的度規就一目了然啦 :P。
